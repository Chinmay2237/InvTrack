...
final createPurchaseOrderUseCaseProvider = Provider<CreatePurchaseOrderUseCase>((ref) {
  final repo = ref.watch(purchaseOrderRepositoryProvider);
  return CreatePurchaseOrderUseCase(repo);
});

final receivePurchaseOrderUseCaseProvider = Provider<ReceivePurchaseOrderUseCase>((ref) {
  final repo = ref.watch(purchaseOrderRepositoryProvider);
  return ReceivePurchaseOrderUseCase(repo);
});

final purchaseOrdersListProvider = StreamProvider<List<PurchaseOrderEntity>>((ref) {
  final repo = ref.watch(purchaseOrderRepositoryProvider);
